      ...
      <div className="space-y-16">
        ...
        <motion.section variants={itemVariants} className="space-y-10">
          ...
          <div className="bg-[#0a0f1d] border border-white/10 rounded-[2rem] p-10 overflow-x-auto font-mono text-base leading-relaxed text-slate-400 shadow-2xl">
            ...
            <p>
              <span className="text-primary">import</span> pandas{" "}
              <span className="text-primary">as</span> pd
            </p>
            <p>
              <span className="text-primary">import</span> docker
            </p>
            <p className="mt-6 text-slate-600">
              # Initialize IoT data pipeline
            </p>
            <p>client = docker.from_env()</p>
            <p>
              container = client.containers.run(
              <span className="text-amber-300">"energy-monitor"</span>, detach=
              <span className="text-primary">True</span>)
            </p>
            <p className="mt-6 text-slate-600"># Process sensor stream</p>
            <p>
              df = pd.read_sql(
              <span className="text-amber-300">"SELECT * FROM sensors"</span>,
              engine)
            </p>
          </div>
        </motion.section>
            ...
            {[
              {
                ...
                title: "18% Faster",
                desc: "Average reduction in peak-hour travel time.",
              },
              {
                icon: <Cloud className="text-primary" />,
                title: "12% Less CO2",
                desc: "Estimated decrease in vehicle emissions.",
              },
              {
                icon: <BarChart className="text-primary" />,
                title: "94% Accuracy",
                desc: "Prediction precision for congestion events.",
              },
            ].map((item, i) => (
              <motion.div
                key={i}
