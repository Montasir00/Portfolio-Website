...
import { ArrowLeft, Download, MapPin, GraduationCap, Award, TrendingUp } from "lucide-react";
...
export const About = ({ onBack }: AboutProps) => {
  const skills = ["Python", "SQL", "PHP", "Pandas", "NumPy", "Matplotlib", "Docker", "Git", "MQTT", "Ethereum", "Web3.php"];
  
  const education = [
    { degree: "BSc in Data Analysis", school: "University of Messina, Italy", year: "Expected Oct 2025" },
  ];
    ...
    >
      ...
      <motion.section variants={itemVariants} className="flex flex-col items-center text-center space-y-8">
        ...
        <div className="space-y-3">
          ...
          <p className="text-primary text-xl font-bold uppercase tracking-widest">Data Analyst & Storyteller</p>
      ...
      <motion.section variants={itemVariants} className="space-y-8">
        ...
        <div className="space-y-6 text-xl text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
          <p>
            I am a dedicated <span className="text-primary font-bold">Data Analysis student</span> at the University of Messina with a passion for uncovering insights from complex environmental and industrial datasets.
          </p>
          <p>
            My experience ranges from analyzing air quality data for the Copernicus Climate Data Store to building secure blockchain transaction systems. I bridge the gap between <span className="text-primary font-bold">technical implementation</span> and data-driven decision making.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 pt-6">
          {skills.map(skill => (
            <motion.span 
              key={skill} 
              whileHover={{ scale: 1.1, backgroundColor: 'var(--color-primary)', color: 'var(--color-ink)' }}
      ...
      {/* Education */}
      <motion.section variants={itemVariants} className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-12 space-y-10 shadow-2xl">
          ...
          {education.map((edu, i) => (
            ...
            >
              <div className="flex-1">
                <p className="text-white font-bold text-2xl tracking-tight">{edu.degree}</p>
                <p className="text-slate-400 text-lg">{edu.school} • {edu.year}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <motion.section variants={itemVariants} className="space-y-8">
        ...
        <div className="grid gap-6">
          {languages.map((lang, i) => (
            <motion.div 
              key={i} 
              whileHover={{ scale: 1.02, borderColor: 'rgba(17, 180, 212, 0.4)' }}
            ...
            >
              ...
              <span className="text-slate-700 dark:text-slate-200 font-bold text-lg tracking-tight">{lang}</span>
            </motion.div>
          ))}
        </div>
      </motion.section>
