...
import { Code, Database, BarChart3, ArrowRight, Download, Mail, ExternalLink, PieChart, Activity } from "lucide-react";
...
export const Home = ({ onNavigate }: HeroProps) => {
  const toolkit = [
    ...
    { name: "Docker", icon: <BarChart3 size={28} />, label: "Containerization" },
  ];
    ...
    >
      ...
      <section className="relative overflow-hidden rounded-[2.5rem] bg-slate-900 border border-primary/20 p-10 md:p-16 text-center shadow-2xl">
        ...
        <div className="relative z-10 space-y-8">
          ...
          >
            ...
            <span className="text-primary text-glow">Extracting Value from Data</span>
          </motion.h1>
          ...
          >
            Hi, I'm Fazlur Rahman. I'm a Data Analysis student at the University of Messina, specializing in environmental data analysis, IoT pipelines, and secure transaction systems.
          </motion.p>
      ...
      <section className="space-y-8">
        <div className="flex items-center justify-between border-b border-primary/10 pb-4">
          <h2 className="text-2xl font-bold tracking-tight font-mono uppercase italic text-primary">Technical Toolkit</h2>
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Core Expertise</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {toolkit.map((item, idx) => (
            ...
            >
              ...
              <span className="font-bold text-xl text-slate-900 dark:text-slate-200">{item.name}</span>
              <span className="text-[10px] text-slate-500 uppercase tracking-[0.3em] mt-2 font-bold">{item.label}</span>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-8">
        <div className="flex items-center justify-between border-b border-primary/10 pb-4">
          <h2 className="text-2xl font-bold tracking-tight font-mono uppercase italic text-primary">Featured Project</h2>
          <button 
        ...
        >
          <div className="h-80 w-full bg-slate-900 relative overflow-hidden">
            ...
            <div className="absolute bottom-8 left-8 z-20 flex gap-4">
              <span className="px-4 py-1.5 bg-slate-900/80 backdrop-blur-md border border-white/10 text-primary text-[10px] font-bold rounded-xl uppercase tracking-widest">Python</span>
              <span className="px-4 py-1.5 bg-slate-900/80 backdrop-blur-md border border-white/10 text-primary text-[10px] font-bold rounded-xl uppercase tracking-widest">Data Analysis</span>
            </div>
          </div>

          <div className="p-10 space-y-6">
            <h3 className="text-3xl font-bold text-slate-900 dark:text-white group-hover:text-primary transition-colors tracking-tight">Weather Forecast Research</h3>
            <p className="text-slate-500 dark:text-slate-400 text-lg leading-relaxed">
              Analysing environmental data from Copernicus Climate Data Store for Milan air quality. Performing correlation analysis between temperature and pollutants using Python.
            </p>
